...
import ants # this takes a while to import
import sys
import nibabel as nib
print('imported stuff!')
print(datetime.now())

df = pd.read_csv ('/mmfs1/data/pijarj/NDAR_BoldAnat10/image03.txt',delimiter='\t',low_memory=False)
df = df.iloc[1::] 
...
safe_mkdir((os.path.join(bids_root,f'ds-{study_id}')))
safe_mkdir(os.path.join(bids_root,f'ds-{study_id}',f'sub-{subject_id+1:03d}'))
safe_mkdir(os.path.join(bids_root,f'ds-{study_id}',f'sub-{subject_id+1:03d}','func'))
safe_mkdir(os.path.join(bids_root,f'ds-{study_id}',f'sub-{subject_id+1:03d}','anat'))

study_df = general_df.iloc[general_df['collection_id'].values==study_id]
n = len(study_df)
...
## Writes json for each individual EPI
